#define _PY_SSIZE_T_CLEAN
#include <Python.h>

#pragma region tmmoc.c
#include <stdio.h>
#include <stdbool.h>
#include <string.h>
#include <ctype.h>
...
static PyObject *
tmmoc_totalMolarMassOfCompound(PyObject* self, PyObject* args)
{
    const char* formula;

    if (!PyArg_ParseTuple(args, "s", &formula))
        return NULL;
    
    double res = totalMolarMassOfCompound(formula);

    if (res == -1)
    {
        PyErr_SetString(PyExc_ValueError, "Invalid formula");
        return NULL;
    }
...
static PyMethodDef TMMOCMethods[] = {
    {"totalMolarMassOfCompound", tmmoc_totalMolarMassOfCompound, METH_VARARGS,
    "Calculate the total molar mass of a compound"},

    {NULL, NULL, 0, NULL}  
};

static struct PyModuleDef tmmocmodule = {
...
PyInit_tmmoc(void)
{
    return PyModule_Create(&tmmocmodule);
}
...
int main(int argc, char** argv)
{
    ...
    if (PyImport_AppendInittab("tmmoc", PyInit_tmmoc) == -1)
    {
    ...
    PyObject* module = PyImport_ImportModule("tmmoc");

    if (!module)
    {
        PyErr_Print();
        fprintf(stderr, "Error: could not import module 'tmmoc'\n");
    }
